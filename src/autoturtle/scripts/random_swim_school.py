...
class SwimSchool():
    ...
    def __init__(self):
        init_posx = random.uniform(0, 10)
        init_posy = random.uniform(0, 10)
        ...
        init_pose = Pose()
        init_pose.x = round(init_posx, 2)
        init_pose.y = round(init_posx, 2)

        init_move = Twist()
        init_move.linear.x = init_posx
        init_move.linear.y = init_posy
        ...
        start = time.time()
        #check = True
        self.cmd_vel.publish(init_move)
